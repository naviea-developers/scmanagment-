...
class ProfLossExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    private $data;
    private $startDate;
    private $endDate;
    public function __construct($transactions,$direct_tans,$indirect_tans , $startDate, $endDate)
    {
        $formattedData = [];
        $total = 0;
        ...
        foreach($transactions as $transaction)
        {
             if($transaction->type == 4){
                ...
                $dr = '--';
                $cr =  $transaction->b_acount;
            }else{
                $total_dr += $transaction->b_acount;
                $dr = $transaction->b_acount;
                $cr =  '--';
            }
            $formattedData[] = [
                'Account Name' => $transaction->account_name,
                'Account Code' =>$transaction->account_code,
                'Type'  =>$transaction->type == 4 ? "Income" : 'Expense',
                'Debit' => 'BDT '.round($dr,2),
                'Credit' =>'BDT '.round($cr,2),
        ...
        if($formattedData != [])
        {
            ...
            if($total_dr > $total_cr){
                $formattedData[] = [
                    'Account Name'         => 'Net Loss',
                    'Account Code'       =>'',
                    'Type' => '',
                    'Debit'  =>'BDT '.round($total_dr-$total_cr,2),
                    'Credit'   =>  ''
                ];
            }else{
                $formattedData[] = [
                    ...
                    'Type' => '',
                    'Debit'  =>'',
                    'Credit'   =>  'BDT '.round($total_cr-$total_dr,2)
                ];
            }
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue('A5', 'Profit & Loss')->getStyle('A5')->getFont()->setBold(true);
                $startRow = 1;
