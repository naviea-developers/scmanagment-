...
class BalanceSheetExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    private $data;
    private $startDate;
    private $endDate;
    public function __construct($transactions , $startDate, $endDate)
    {
        ...
        foreach($transactions as $transaction)
        {
            if($transaction->type == 1){
                ...
                $formattedData[] = [
                    'Asset'         => $transaction->account_name,
                    'Ammount'       =>'BDT'.' '.round($transaction->b_acount,2),
                    'Liability & Equity'  => '',
                    'Amount'   =>'',
                ...
                $formattedData[] = [
                    'Asset'         => '',
                    'Ammount'       =>'',
                    'Liability & Equity'  => $transaction->account_name,
                    'Amount'   =>'BDT'.' '.round($transaction->b_acount,2)
        ...
        if($formattedData != [])
        {
            $formattedData[] = [
                'Asset'         => 'Total',
                'Ammount'       =>'BDT'.' '.round($total_a, 2),
                'Liability & Equity'  =>'',
                'Amount'   => 'BDT'.' '.round($total_l, 2)
            ];
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue('A5', 'Balance Sheet')->getStyle('A5')->getFont()->setBold(true);
                $startRow = 1;
