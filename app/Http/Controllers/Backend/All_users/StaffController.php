...
class StaffController extends Controller
{
    public function index()
    {
        ...
        // dd($data);
        return view("Backend.all_users.staff.manage",$data);
    }
    ...
    function ajaxData(Request $request){
        ...
        $totalData = User::where('type', 8)->count();
        ...
        if(!empty($datalist))
        {
            ...
            foreach($datalist as $data_v)
            {
                ...
                $nestedData['designation'] = @$data_v->user_designation->name;
                $nestedData['mobile'] = @$data_v->mobile;
                $nestedData['email'] = @$data_v->email;
                $nestedData['gender'] = ($data_v->gender == '0') ? 'Male' : 'Female';
                $nestedData['status'] = '';
                if ($data_v->status == 0) {
                    $nestedData['status'] .= '<a href="'.route('admin.staff.status', $data_v->id).'" class="data_status btn btn-sm btn-warning">Inactive</a>';
                } elseif ($data_v->status == 1) {
                    $nestedData['status'] .= '<a href="'.route('admin.staff.status', $data_v->id).'" class="data_status btn btn-sm btn-success">Active</a>';
                }
 
                $nestedData['options'] = '<a class="btn btn-primary data_edit" href="'.route('admin.staff.edit', $data_v->id).'"><i class="fa fa-edit"></i></a>';
