...
class BlogController extends Controller
{
    ...
    public function store(Request $request)
    {
       // dd($request->all());
        ...
        return redirect()->route('blog.index')->with('message','Blog Add Successfully');
    }
    ...
    public function blogCommentUpdate(Request $request, $id)
    {
        ...
        $blog_comment->update();
        return redirect()->route('blog.comments')->with('message', 'Comment Update Successfully, Thank you.');
    }
    public function blogCommentDelete(Request $request)
    {
        $blog_comment= Comment::find($request->blog_comment_id);
        $blog_comment->delete();
        return redirect()->back()->with('message', 'Blog Delete Successfully, Thank You.');
    }
    ...
    public function addSelect2(Request $request){
     ...
     public function storeBlogTopic(Request $request)
     {
        ...
        $topic->type = 'blog';
        $topic->save();
        return redirect()->back()->with('message', 'Topic added Successfully, Thank you.');
     }
     ...
     public function updateBlogTopic(Request $request, $id)
     {
        ...
        $topic->type = 'blog';
        $topic->update();
        return redirect()->route('blog.manage_topic')->with('message', 'Topic added Successfully, Thank you.');
     }
     public function status_toggleBlogTopic($id)
     {
         ...
         $topic->update();
         return redirect()->route('blog.manage_topic')->with('message', 'Status Update Successfully, Thank you.');
     }
     public function destroyBlogTopic(Request $request)
     {
         $topic= Topic::find($request->topic_id);
         $topic->delete();
         return redirect()->back()->with('message', 'Topic Delete Successfully, Thank You.');
     }
