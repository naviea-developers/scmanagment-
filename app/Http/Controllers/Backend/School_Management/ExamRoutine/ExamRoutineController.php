...
class ExamRoutineController extends Controller
{
    ...
    public function getExamRoutine($examination_id, $session_id) {
        $data['tpOption'] =Tp_option::where('option_name', 'theme_option_header')->first();

        if($examination_id && $session_id){
        $data['examSchedules']= ExamSchedule::where('examination_id', $examination_id)
                                  ->whereHas('examination', function($query) use ($session_id) {
