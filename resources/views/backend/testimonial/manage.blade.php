                ...
                <tbody>
                  ...
                  @if (count($testimonials) > 0)
                    @foreach ($testimonials as $testimonial)
                      <tr>
                          <td>{{ $i++ }}</td>
                          <td> <img src="{{ $testimonial->image_show }}" alt="" width="60px" height="40px" srcset=""></td>
                          <td>{{ $testimonial->name }}</td>
                          <td>{{ $testimonial->designation }}</td>
                          <td>{{ $testimonial->star }}</td>
                          <td>
                            @if($testimonial->status == 0)
                            <a href="{{ route('admin.status_testimonial',$testimonial->id) }}" class="btn btn-sm btn-warning">Inactive</a>
                            @elseif($testimonial->status == 1)
                            <a href="{{ route('admin.status_testimonial',$testimonial->id) }}" class="btn btn-sm btn-success">Active</a>
                            @endif
                          </td>
                          <td>
                            <a class="btn text-info" href="{{ route('admin.edit_testimonial', $testimonial->id) }}"><i class="icon ion-compose tx-28"></i></a>
                            <button class="btn text-danger bg-white"  value="{{$testimonial->id}}" id="homeCatdelete"><i class="icon ion-trash-a tx-28"></i></button>
                          </td>
                      </tr>
                    @endforeach
                  @endif --}}
