                  ...
                  @if (count($categories) > 0)
                    @foreach ($categories as $cat)
                      <tr>
                          <td>{{ $i++ }}</td>
                          <td>
                            <img src="{{ $cat->image_show }}" alt="" width="60px" height="40px" srcset="">
                          </td>
                          <td>{{ $cat->name }}</td>
                          <td>
                            @if ($cat->type == 'home')
                              Course
                            @elseif ($cat->type == 'event')
                              Event
                            @elseif ($cat->type == 'blog')
                              Blog
                            @elseif ($cat->type == 'master_course')
                              Master Course
                              @elseif ($cat->type == 'ebook')
                              Ebook
                            @endif
                          </td>
                          <td>
                          ...
                          <td>
                            <a class="btn text-info" href="{{ route('home-category.edit', $cat->id) }}"><i class="icon ion-compose tx-28"></i></a>
    ...
    <div id="homeDeleteCategory" class="modal fade">
        ...
        <div class="modal-content tx-size-sm">
            <div class="modal-body tx-center pd-y-20 pd-x-20">
                <form action="{{ route('home-category.delete') }}" method="post">
                    @csrf
