                  ...
                  @if (count($alumnis) > 0)
                    @foreach ($alumnis as $alumni)
                      <tr>
                          ...
                          <td>
                            @if(@$alumni->status == 0)
                            <a href="{{ route('admin.alumni.status',$alumni->id) }}" class="btn btn-sm btn-warning">Inactive</a>
                            @elseif(@$alumni->status == 1)
                            <a href="{{ route('admin.alumni.status',$alumni->id) }}" class="btn btn-sm btn-success">Active</a>
                            @endif
                          </td>
                          {{-- <td>{{ $alumni->mobile }}</td>
                          <td>{{ $alumni->email }}</td> --}}
    ...
    <div id="changePassword" class="modal fade">
      ...
      <div class="modal-content tx-size-sm">
          <div class="modal-bodypd-y-20 pd-x-20 p-4">
              <form action="{{ route('admin.alumni_change_password') }}" method="post">
                  @csrf
