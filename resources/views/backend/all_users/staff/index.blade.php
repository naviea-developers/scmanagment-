                  ...
                  @if (count($staffes) > 0)
                    @foreach ($staffes as $staff)
                      <tr>
                          <td>{{ $i++ }}</td>
                          <td>
                            <img src="{{$staff->image_show}}" alt="" width="60px" height="40px" srcset="">
                          </td>
                          <td>{{ $staff->name }}</td>
                          <td>{{ @$staff->user_designation->name }}</td>
                          {{-- <td>
                          ...
                          <td>
                            @if($staff->status == 0)
                            <a href="{{ route('admin.staff.status',$staff->id) }}" class="btn btn-sm btn-warning">Inactive</a>
                            @elseif($staff->status == 1)
                            <a href="{{ route('admin.staff.status',$staff->id) }}" class="btn btn-sm btn-success">Active</a>
                            @endif
                          </td>
                          <td>
                            <a class="btn text-info" href="{{ route('admin.staff.edit', $staff->id) }}"><i class="icon ion-compose tx-28"></i></a>
                            <button class="btn text-danger bg-white"  value="{{$staff->id}}" id="dataDeleteModal"><i class="icon ion-trash-a tx-28"></i></button>
                            <button class="btn text-white bg-info changePass"  value="{{$staff->id}}">Change Password</button>
                          </td>
                      </tr>
                    @endforeach
                  @endif
    ...
    <div id="changePassword" class="modal fade">
      ...
      <div class="modal-content tx-size-sm">
          <div class="modal-bodypd-y-20 pd-x-20 p-4">
              <form action="{{ route('admin.staff_change_password') }}" method="post">
                  @csrf
