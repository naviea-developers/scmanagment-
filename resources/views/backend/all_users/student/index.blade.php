                  ...
                  @if (count($students) > 0)
                    @foreach ($students as $student)
                      <tr>
                          ...
                          <td>{{ $student->name }}</td>
                          <td>
                            <img src="{{$student->image_show}}" alt="" width="60px" height="40px" srcset="">
                          </td>
                          ...
                          <td>
                            <a class="btn text-info" href="{{ route('admin.student_details', $student->id) }}"><i class="fa fa-solid fa-eye tx-28"></i></a>
                            <a class="btn text-info" href="{{ route('admin.student.edit', $student->id) }}"><i class="icon ion-compose tx-28"></i></a>
                            <button class="btn text-danger bg-white"  value="{{$student->id}}" id="dataDeleteModal"><i class="icon ion-trash-a tx-28"></i></button>
                            <button class="btn text-white bg-info changePass"  value="{{$student->id}}">Change Password</button>
                          </td>
                      </tr>
                    @endforeach
                  @endif
    ...
    <div id="changePassword" class="modal fade">
      ...
      <div class="modal-content tx-size-sm">
          <div class="modal-bodypd-y-20 pd-x-20 p-4">
              <form action="{{ route('admin.student_change_password') }}" method="post">
                  @csrf
