                  ...
                  @if (count($admins) > 0)
                    @foreach ($admins as $admin)
                      <tr>
                          ...
                          <td>{{ $admin->name }}</td>
                          <td>
                            <img src="{{$admin->image_show}}" alt="" width="60px" height="40px" srcset="">
                          </td>
                          <td>{{ $admin->mobile }}</td>
                          <td>{{ $admin->email }}</td>
                          
                          <td>
                            <a class="btn text-info" href="{{ route('backendog.edit_admin', $admin->id) }}"><i class="icon ion-compose tx-28"></i></a>
                            <button class="btn text-danger bg-white"  value="{{$admin->id}}" id="dataDeleteModal"><i class="icon ion-trash-a tx-28"></i></button>
                            <button class="btn text-white bg-info changePass"  value="{{$admin->id}}">Change Password</button>
                          </td>
                      </tr>
                    @endforeach
                  @endif
    ...
    <div id="datamodalshow" class="modal fade">
        ...
        <div class="modal-content tx-size-sm">
            <div class="modal-body tx-center pd-y-20 pd-x-20">
                <form action="{{ route('backend.delete_admin') }}" method="post">
                    @csrf
                    ...
                    <h4 class="tx-danger  tx-semibold mg-b-20 mt-2">Are you sure! you want to delete this?</h4>
                     <input type="hidden" name="admin_id" id="modal_data_id">
