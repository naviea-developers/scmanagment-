    ...
    <div class="br-mainpanel">
        ...
        <div class="br-pagebody">
          <div class="br-section-wrapper">
            <h6 class="br-section-label text-center"> Role</h6>
                  ...
                  @if (count($roles) > 0)
                    @foreach ($roles as $role)
                      <tr>
                          ...
                          <td>
                            @if($role->status == 0)
                            <a href="{{ route('admin.role.status_toggle',$role->id) }}" class="btn btn-sm btn-warning">Inactive</a>
                            @elseif($role->status == 1)
                            <a href="{{ route('admin.role.status_toggle',$role->id) }}" class="btn btn-sm btn-success">Active</a>
                            @endif
                          </td>
                          <td>
                            <a class="btn text-info" href="{{ route('admin.role.edit', $role->id) }}"><i class="icon ion-compose tx-28"></i></a>
                            <button class="btn text-danger bg-white"  value="{{$role->id}}" id="homeCatdelete"><i class="icon ion-trash-a tx-28"></i></button>
                            <button onclick="window.location='{{ route('admin.role.permission',$role->id) }}'" class="btn text-info bg-white">Permission</button>
                          </td>
                      </tr>
                    @endforeach
                  @endif
            ...
            <div class="modal-body tx-center pd-y-20 pd-x-20">
                <form action="{{ route('admin.role.delete') }}" method="post">
                    ...
                    <h4 class="tx-danger  tx-semibold mg-b-20 mt-2">Are you sure! you want to delete this?</h4>
                     <input type="hidden" name="role_id" id="category_id">
