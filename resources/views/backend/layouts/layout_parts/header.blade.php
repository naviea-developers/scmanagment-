        ...
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-header">
            ...
            <div class="media-list">
             ...
             @endphp --}}
            @php
            $notifications = \App\Models\Notification::where('user_id',auth()->guard('admin')->user()->id)->orderBy('id','desc')->take(4)->get();
            @endphp
              @foreach ($notifications as $notification)
                @if ($notification->type=='course')
                  <a href="" class="media-list-link @if($notification->is_read == 0)unread @else read @endif">
                    <div class="media">
                      <img src="{{ @$notification?->cart?->course?->teacher?->image_show }}" alt="">
                      <div class="media-body">
                        <p class="noti-text"> {{ $notification->text }}.</p>
                        <p class="noti-text"> {{ @$notification->cart->course->name}}.</p>
                ...
                @elseif ($notification->type=='university')
                  @foreach (@$notification->application->carts  as $cart) 
                    <a href="{{ route('admin.student_appliction_details',@$notification->application->id) }}" class="media-list-link @if($notification->is_read == 0)unread @else read @endif">
                      <div class="media">
                        <img src="{{ @$cart->course->university->image_show  }}" alt="">
                        <div class="media-body">
                          <p class="noti-text"> {{ @$notification->text }}.</p>
                          <p class="noti-text"> {{ @$cart->course->name}}.</p>
                          <span>{{date('F,d,Y H:i:s',strtotime($notification->created_at))}}</span>
                        </div>
                      </div><!-- media -->
                    </a>
