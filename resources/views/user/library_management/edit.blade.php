...
<div class="right_section">
    <div>
        <h4 style="color: black">Edit Borrowed Information</h4>
    </div>
</div>
<div class="passwodBox mb-3" style="background-color: var(--seller_frontend_color); color:white">
    <form action="{{ route('teacher.library_borrow.update', $borrow->id) }}" method="POST" style="margin-top: 0%">
        @csrf
                ...
                <div class="col-md-6">
                    ...
                    <div class="mg-t-10 mg-sm-t-0">
                        <select id="class-select" name="class_id" class="form-control select2 form-select">
                            ...
                            <option @if ($class->id == $borrow->class_id) selected @endif value="{{ $class->id }}">{{ $class->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mg-t-10 mg-sm-t-0">
                        ...
                        {{-- <select id="student-select" name="student_id" class="form-control form-select select2"> --}}
                            ...
                            <option @if ($student->id == $borrow->student_id) selected @endif value="{{ $student->id }}" data-class-id="{{ $student->class_id }}" data-student-id-number="{{ $student->student_id_number }}">
                ...
                <div class="col-md-6">
                    ...
                    <div class="d-flex">
                        <div class="col-md-8">
                            ...
                            <select id="book-select" name="book_id" class="form-control select2">
                                <option value=""> Select Book</option>
                                @foreach ($books as $book)
                                <option value="{{ $book->id }}" data-book-code="{{ $book->book_code }}" data-class-id="{{ $book->class_id }}">
                                    {{ $book->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div id="selected-books-list" class="col-md-12 mt-3">
                        <!-- Existing borrowed books will appear here -->
                        @foreach ($borrow->borrowItems as $item)
                        <div id="book-{{ $item->book->id }}" class="row mt-2">
                            <div class="col-md-3">{{ @$item->book->book_code }}</div>
                            <div class="col-md-6">{{ @$item->book->name }}</div>
                            <div class="col-md-2">
                                <button type="button" class="btn btn-danger btn-sm remove-book-btn" data-book-id="{{ $item->book->id }}">
                                    X
                                </button>
                            </div>
                        </div>
                        <input type="hidden" name="book_id[]" id="input-book-{{ $item->book->id }}" value="{{ $item->book->id }}">
                        @endforeach
                    </div>

                    <!-- Hidden inputs for newly selected books -->
...
$(document).ready(function() {
    ...
    function filterBooksByClass(classId) {
        ...
        $('#book-select').val('').trigger('change');
    }
    ...
    function updateSelectionsByStudentIdNumber(studentIdNumber) {
        $('#student-select option').each(function() {
            if ($(this).data('student-id-number') == studentIdNumber) {
                ...
                $('#student-select').val($(this).val()).trigger('change');
